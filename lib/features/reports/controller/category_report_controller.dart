...
class CategoryReportController {
  ...
  Future<void> generateCategoryReport({
    ...
    final topQuizzes = _calculateTopQuizzes(quizScores);
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          _buildCategoryOverview(category, quizScores, metrics),
          _buildRadialGauges(metrics),
          _buildTopQuizzes(topQuizzes),
    ...
    _downloadPDF(pdfBytes, 'Category_Report_${category.name}.pdf');
  }

  pw.Widget _buildCategoryHeader(CategoryModel category) {
    return pw.Container(
  ...
  pw.Widget _buildCategoryOverview(CategoryModel category,
      List<QuizScoreModel> scores, PerformanceMetrics metrics) {
    final totalParticipants = scores.map((s) => s.userId).toSet().length;
    final totalQuizzes = scores.map((s) => s.quizId).toSet().length;
            ...
            child: pw.Column(
              children: [
                ...
                pw.Row(
                  mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                  children: [
                    _buildOverviewItem('Total Quizzes', '$totalQuizzes'),
                    _buildOverviewItem(
                        'Total Participants', '$totalParticipants'),
                    _buildOverviewItem('Avg. Category Score',
  ...
  pw.Widget _buildOverviewItem(String title, String value) {
    return pw.Column(
      children: [
        pw.Text(
          title,
  ...
  pw.Widget _buildTopQuizzes(Map<String, QuizStats> topQuizzes) {
    return pw.Container(
      margin: const pw.EdgeInsets.only(top: 24),
      child: pw.Column(
        crossAxisAlignment: pw.CrossAxisAlignment.start,
        children: [
          ...
          pw.SizedBox(height: 16),
          ...topQuizzes.entries
              .map((entry) => _buildQuizCard(entry.key, entry.value)),
        ],
      ),
    );
  }

  pw.Widget _buildQuizCard(String quizTitle, QuizStats stats) {
    return pw.Container(
        ...
        children: [
          ...
          pw.Padding(
            ...
            child: pw.Row(
              ...
              children: [
                _buildQuizStat('Attempts', '${stats.totalAttempts}'),
                _buildQuizStat('Highest Score', '${stats.highestScore}%'),
                _buildQuizStat(
                    'Pass Rate', '${stats.passRate.toStringAsFixed(1)}%'),
              ],
            ),
          ),
        ],
  ...
  pw.Widget _buildQuizStat(String label, String value) {
    return pw.Column(
      children: [
        pw.Text(
          label,
  ...
  Map<String, QuizStats> _calculateTopQuizzes(List<QuizScoreModel> scores) {
    final quizMap = <String, QuizStats>{};

    for (final score in scores) {
      final quizTitle = score.quizTitle ?? 'Untitled Quiz';
      final performance = (score.score / score.totalScore) * 100;
      ...
      quizMap.update(
        ...
        (stats) => QuizStats(
          ...
          averageScore:
              (stats.averageScore * stats.totalAttempts + performance) /
                  (stats.totalAttempts + 1),
          highestScore: performance > stats.highestScore
              ? performance
              : stats.highestScore,
          passRate: performance >= 70 ? stats.passRate + 1 : stats.passRate,
        ),
        ifAbsent: () => QuizStats(
          totalAttempts: 1,
          averageScore: performance,
          highestScore: performance,
          passRate: performance >= 70 ? 1 : 0,
        ),
      );
    }

    // Sort quizzes by average score
    final sortedEntries = quizMap.entries.toList()
      ..sort((a, b) => b.value.averageScore.compareTo(a.value.averageScore));

    return Map.fromEntries(sortedEntries.take(5));
  }

  Map<String, int> _calculatePerformanceDistribution(
